...
namespace fnc {
	...
	//Рекурсивная функция, вычисляет определитель двумерного массива arr размера size;
	double calcDet(std::vector<std::vector<double>> arr, int size) {
		std::cout << size;
		...
		for (int j = 0; j < size; j++) {
			...
			det += arr[i][j] * calcDet(fnc::getAlgAdd(arr, i, j), size - 1);
		}
	...
	void coutBasicInfo(std::vector<std::vector<double>>& arr, int rows, int columns, int id) {
		std::cout<<"ID = " << id << " || Размер = " << rows << "x" << columns << " || Элементы = ";
		std::cout << "{";

		for (int i = 0; i < rows; i++) {
			std::cout << "{";
			for (int j = 0; j < columns; j++) {
				if (j != columns - 1) std::cout << arr[i][j] << ", ";
	...
	void checkBase(bool& log, bool& rewrite) {
		fnc::coutArr({ " >>> Вывод матрицы в консоль: ", "1. Вывести матрицу в консоль после создания", "2. Не выводить матрицу" });
